...
public class BoardGUI {
    private final String[] playerImages =
            {"head1.png", "head2.png", "head3.png", "head4.png"};
    private final Image[] statusEffects =
            {new Image("fireEffect.png"), new Image("iceEffect.png")};
    private final Image fixedImage = new Image("F.png");
    @FXML
    public Button nextTurnButton;
    @FXML
    public BorderPane baseBoarderPane;
    @FXML
    public Label playerTurnTag;
    @FXML
    ...
    private void drawCanvas() {
        ...
        for (int i = 0; i < boardX; i++) {
            for (int j = 0; j < boardY; j++) {
                ...
                if ((i == mouseX - 1) && (j == mouseY - 1)) {
                    ...
                    gc.fillRect((boxX * i) + xPad + 1, (boxY * j) + yPad + 1,
                            boxX - 2, boxY - 2);
                }
                //get tile
                ...
                if (t != null) {
                    ...
                    Rotate rotation = new Rotate(t.getRotation(),
                            (boxX * (i + 0.5)) + xPad,
                            (boxY * (j + 0.5)) + yPad);
                    gc.save();
                    gc.setTransform(rotation.getMxx(), rotation.getMyx(),
                            rotation.getMxy(), rotation.getMyy(),
                            rotation.getTx(), rotation.getTy());
                    gc.drawImage(tileImage, (boxX * i) + xPad + 2,
                            (boxY * j) + yPad + 2, boxX - 4, boxY - 4);
                    gc.restore();
                    //draw status effects
                    if (t.getIsOnFire()) {
                        gc.drawImage(statusEffects[0], (boxX * i) + xPad + 2,
                                (boxY * j) + yPad + 2, boxX - 4, boxY - 4);
                    } else if (t.getIsFrozen()) {
                        gc.drawImage(statusEffects[1], (boxX * i) + xPad + 2,
                                (boxY * j) + yPad + 2, boxX - 4, boxY - 4);
                    }
                    if (t.getIsTileFixed()) {
                        gc.drawImage(fixedImage, (boxX * i) + xPad + 2,
                                (boxY * j) + yPad + 2, boxX - 4, boxY - 4);
                    }
                } else {
                    //if tile is null draw a blue square
                    gc.setFill(Color.BLUE);
                    gc.fillRect((boxX * i) + xPad + 2, (boxY * j) + yPad + 2,
                            boxX - 4, boxY - 4);
                }
            }
        }
            ...
            for (int i = 0; i < (boardX); i++) {
                if (this.game.getBoard().isColumnPushable(i)) {
                    ...
                    if (i == mouseX - 1 && 0 == mouseY) {
                        gc.setFill(Color.RED);
                        gc.fillPolygon(new double[] {(boxX * i) + xPad,
                                        (boxX * i) + xPad + (boxX / 2),
                                        (boxX * (i + 1)) + xPad},
                                new double[] {yPad - boxY, yPad, yPad - boxY},
                                3);
                        gc.setFill(Color.YELLOW);
                    }
                    gc.fillPolygon(new double[] {(boxX * i) + xPad + 2,
                                    (boxX * i) + xPad + (boxX / 2),
                                    (boxX * (i + 1)) + xPad - 2},
                            new double[] {yPad - boxY + 2, yPad - 2,
                                    yPad - boxY + 2}, 3);
            ...
            for (int i = 0; i < (boardY); i++) {
                if (this.game.getBoard().isRowPushable(i)) {
                    ...
                    if (0 == mouseX && i == mouseY - 1) {
                        gc.setFill(Color.RED);
                        gc.fillPolygon(
                                new double[] {xPad - boxX, xPad, xPad - boxX},
                                new double[] {(boxY * i) + yPad,
                                        (boxY * i) + yPad + (boxY / 2),
                                        (boxY * (i + 1)) + yPad}, 3);
                        gc.setFill(Color.YELLOW);
                    }
                    gc.fillPolygon(new double[] {xPad - boxX + 2, xPad - 2,
                                    xPad - boxX + 2},
                            new double[] {(boxY * i) + yPad + 2,
                                    (boxY * i) + yPad + (boxY / 2),
                                    (boxY * (i + 1)) + yPad - 2}, 3);
                    //add right facing triangle#
                    //check if selected
                    ...
                    gc.fillPolygon(
                            new double[] {xPad + (boxX * (boardX + 1)) - 2,
                                    xPad + (boxX * boardX) + 2,
                                    xPad + (boxX * (boardX + 1)) - 2},
                            new double[] {(boxY * i) + yPad + 2,
                                    (boxY * i) + yPad + (boxY / 2),
                                    (boxY * (i + 1)) + yPad - 2}, 3);
                }
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            gc.drawImage(playerImage,
                    (boxX * x) + xPad + ((boxX / 4) * (subx + 1)),
                    (boxY * y) + yPad + ((boxY / 4) * (suby + 1)), boxX / 4,
                    boxY / 4);
        }
    }
    ...
    public void canvasClickEventHandler(double x, double y) {
        ...
        if ((x > xPad - boxX) && (y > yPad - boxY)) {
            if ((x < (xPad + (boxX * (boardX + 1)))) &&
                    (y < (yPad + (boxY * (boardY + 1))))) {
                x = x - xPad + boxX;
                y = y - yPad + boxY;
    ...
    public void playerPushInTile() {
        ...
        //get selected tile
        FloorTile t =
                (FloorTile) this.game.getTurn().getHand().get(this.handIndex);
        //push in the tile
        ...
        if (mouseX == 0 || mouseX == boardX + 1) {
            if (mouseY > 0 && mouseY < boardY + 1) {
                ...
                if (this.game.getBoard().isRowPushable((int) mouseY - 1)) {
                    FloorTile tile = game.getBoard()
                            .pushInRow(t, (int) mouseY - 1, forward);
            ...
            if (mouseX > 0 && mouseX < boardX + 1) {
                ...
                if (this.game.getBoard().isColumnPushable((int) mouseX - 1)) {
                    FloorTile tile = this.game.getBoard()
                            .pushInColumn(t, (int) mouseX - 1, forward);
                    this.game.getBag().addTile(tile);
    ...
    public void startTurn() {
        ...
        if (drawnTile instanceof ActionTile){
            ...
            if (canPlaceTile()) {
                ...
                if (p.getHand().size() > 1) {
                    playerTurnTag
                            .setText("Do you want to play the action Tile");
                    turnProgression = 1;
                } else {
                    //allow them to move
                    isAbleToMove(p);
    ...
    public void handClicked(int index) {
        ...
        if (selectedTile instanceof ActionTile && turnProgression == 1) {
            ...
            //must play this now
            playerTurnTag.setText(
                    "Set the rotation and click a triangle to push in from");
    ...
    public int[] canMoveTo(Player p, double x, double y) {
        ...
        if (x >= 0 && x < boardX && y >= 0 && y < boardY) {
            if (this.game.getBoard()
                    .isAccessibleFrom(p.getX(), p.getY(), newPos[0],
                            newPos[1])) {
    ...
    public void leftRotateButtonAction(ActionEvent actionEvent) {
        Game.playMenuSound();
        if (turnProgression == 0) {
            FloorTile tempTile = (FloorTile) this.game.getTurn().getHand()
                    .get(this.handIndex);
    ...
    public void setCards(ArrayList<Tile> deck) {
        ...
        if (deck.size() == 0) {
            ...
            for (Tile card : deck) {
                Image cardImage =
                        new Image(card.getImageLocation(), 40, 40, true, true);
                Image borderImage =
                        new Image("invSlot.png", 64, 64, true, true);
