...
public class Game extends Application {
    ...
    private static final File MENU_SOUND_FILE_PATH =
            new File("sounds/menuSound.mp3");
    private static final File FIRE_SOUND_FILE_PATH =
            new File("sounds/fireSound.mp3");
    private static final File ICE_SOUND_FILE_PATH =
            new File("sounds/iceSound.mp3");
    ...
    private static final Media media =
            new Media(new File(MUSIC_FILE_PATH).toURI().toString());
    private static final MediaPlayer mediaPlayer = new MediaPlayer(media);
    private static final AudioClip menuSound = new AudioClip(
            Paths.get(MENU_SOUND_FILE_PATH.toString()).toUri().toString());
    ...
    public void playActionSound(String tileType) {
        ...
        if (tileType.equals("FireTile")) {
            ...
            AudioClip fire = new AudioClip(
                    Paths.get(FIRE_SOUND_FILE_PATH.getPath()).toUri()
                            .toString());
            ...
            AudioClip ice = new AudioClip(
                    Paths.get(ICE_SOUND_FILE_PATH.getPath()).toUri()
                            .toString());
    ...
    public void saveBoard(String filePath) {
        ...
        try {
            ...
            fileWriter.write(this.board.getLength() + "," +
                    this.board.getWidth() + "\n");
            ...
            for (Player p : this.players) {
                fileWriter.write(p.getX() + "," + p.getY() + "," +
                        p.getProfile() + "," + p.getNumOfTiles() + "\n");
                for (Tile t : p.getHand()) {
                    fileWriter.write(t.getTILETYPE() + "\n");
                }
                //write previous positions
                fileWriter.write(p.getPreviousPosition()[0] + "," +
                        p.getPreviousPosition()[1] + "\n");
                fileWriter.write(p.getPreviousPosition2()[0] + "," +
                        p.getPreviousPosition2()[1] + "\n");
            }
    ...
    public void saveProfiles() {
        ...
        try {
            ...
            for (Profile profile : this.profiles) {
                ...
                fileWriter.write(profile.getName() + "," +
